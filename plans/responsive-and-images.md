...
| # | Criterion | Status |
|---|---|---|
| AC1 | No horizontal scroll at 320–1440px on all 7 routes | **Met in code** — the `lg:-right-20` overhang that caused it is gone and `overflow-x: clip` guards the rest. Not yet confirmed visually (see §6). |
| AC2 | Fully navigable at 360px | **Met** — `SiteNav` drawer reaches all 5 sub-pages and all section anchors. |
| AC3 | Scholar metrics visible at every breakpoint | **Met** — card is a grid item, no longer `hidden lg:block`. |
| AC4 | No image displayed above its intrinsic width at 2× DPR | **Partly met** — 10 of 14 pass. The 4 documentary photos fail and are blocked on originals; `npm run check:images` exits non-zero until they land. |
| AC5 | Every image has dimensions, `loading`, `decoding` | **Met** — via `Img`; the three remote logos were given the same attributes by hand. |
| AC6 | Homepage image payload < 500 KB | **Met, by a wide margin** — 3,494 KB → 68 KB (98% smaller). Above the fold it is 20 KB; the gallery images are now lazy. |
| AC7 | No text under 14px; tap targets ≥ 44px | **Met** for nav, buttons, carousel dots, form controls, and drawer rows. Year labels under the citation chart are 10px, which is deliberate for axis ticks. |
| AC8 | Build succeeds; lint introduces no new errors | **Met** — `npm run build` passes. Lint was crashing on `main`; after bumping typescript-eslint it reports 5 errors and 7 warnings, all pre-existing, none in new files. |

## 5. Outcome

19 commits. Headline results:

- **The site is now usable on a phone at all.** It previously had no mobile
  navigation whatsoever — the single worst defect found.
- **Homepage images: 3,494 KB → 68 KB.** `profile_image.jpg` alone went from
  3,059 KB to 7.8 KB at the size it actually renders.
- **The hero no longer looks washed out** — its image was being drawn at
  `opacity-10`.
- Three logic bugs fixed in passing: a typing effect that re-ran on every render,
  a citation chart hard-coded to a 300 ceiling that 2025's 294 nearly overflowed,
  and two sparse-array holes in the publication data.

## 6. Still outstanding

1. **Four documentary photos need originals from Dr. Mukherjee** — `image1`
   (200px), `image4` (275px), `image6` (213px), `image2` (768px). No better
   copies exist publicly; see `public/images/CREDITS.md` for what was searched.
2. **Visual confirmation at each breakpoint has not been done.** The
   `chrome-devtools` MCP was configured after this session started, so it was not
   loaded and no screenshots were possible. Everything here was verified by
   reading the code, type-checking, building, and serving the production bundle.
   A visual pass should follow.
3. **Gallery captions do not match their photographs** (a lecture captioned
   "Quantum Computing Lab", an award ceremony captioned "Security Seminar"). This
   is content, not layout — it needs someone who knows what the photos show.
4. **The contact form posts nowhere.** It has no handler and no endpoint.
5. **The JS bundle is a single 585 KB chunk** (180 KB gzipped). Route-level code
   splitting would cut the homepage's share substantially.
6. **`scholarService.js` returns `since2020: 0`** for all three metrics.
7. `dist/images` ships both the originals and the derivatives (~12 MB). No user
   downloads the unreferenced originals, so this costs deploy size only.
